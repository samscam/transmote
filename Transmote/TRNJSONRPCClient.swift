...
class TRNJSONRPCClient: AFJSONRPCClient {
    ...
    convenience init(endpointURL URL: URL) {
        if (super.init(endpointURL: URL)) {
            self.responseSerializer.acceptableContentTypes = Set<AnyHashable>(["application/json", "text/html"])
        }
    }
    /** 
